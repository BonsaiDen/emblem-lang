...
Node('Assign', function(target, expr, operator) {
    ...
    init: function() {
        ...
        if (this.tType.hasEmptyType(this.eType)) {
            this.eType = this.tType;
        }

        if (this.access) {

            // Type of the value resulting from the access
            var accessType = this.access.getType(this.tType, this.eType);
    ...
    toString: function() {
        ...
        if (this.access) {
            ...
            // TODO complete!!!!!
            var accessType = this.access.getType(this.tType, this.eType);
            var t = accessType.getAssignmentTemplate(this.op, this.eType);
