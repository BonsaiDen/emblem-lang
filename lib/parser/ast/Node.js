...
function AstNode(id, ctor, proto, statement, block) {
    ...
    var node = function AstNode(args) {
...
AstNode.prototype = {
    ...
    // AST Helpers ------------------------------------------------------------
    isType: function() {
        throw new TypeError('AST Node used in place of TypeDescriptor');
    },
    ...
    // Other Helpers ----------------------------------------------------------
    each: function(list, func, scope) {
        var that = this;
        list.forEach(function() {
            func.apply(scope || that, arguments);
        });
    },

    map: function(list, func, scope) {
        var that = this;
        return list.map(function() {
            return func.apply(scope || that, arguments);
        });
    },
