...
var Parser = function(source) {
    ...
    function parseLine() {
        ...
        // Type declarations
            ...
            stmt = parseTypeDeclaration(true, true);
            expectNewline();
    ...
    // Declarations -----------------------------------------------------------
    // ------------------------------------------------------------------------
    function parseTypeDeclaration(allowFunctions, allowValue) {

        // TODO allow void type for functions...
        ...
        if (allowFunctions && match('(')) {
            ...
            while(true) {
                ...
                params.push(parseTypeDeclaration(false, false));
                if (!match(',')) {
                    break;
            ...
            return node.Function(typeDesc, name, parseBlock());
    ...
    function parseTypeDesc() {
        ...
        if (inType) {
            ...
            // TODO remove type desc?
            type = node.TypeDesc(parseType(true, modifier));

        } else {
            type = node.TypeDesc(parseType(true));
        }
        ...
        if (match('(')) {
            ...
            return node.TypeDesc(node.Type('function', [type, params]));
