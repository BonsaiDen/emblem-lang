...
TypeDescriptor.prototype = {
    ...
    getIndexGetReturn: function(indexType) {
        return Type.getTypeDescriptor(this.getIndexParam(indexType, null, null, 'returnType'));
    },

    getIndexGetTemplate: function(indexType) {
        return this.getIndexParam(indexType, null, null, 'template');
    },

    defineIndexSetOp: function(op, indexType, assignType, returnType, template) {
        this.operators.index.set[indexType + op + assignType] = {
            returnType: returnType,
            template: template
        };
    },

    getIndexSetReturn: function(op, indexType, assignType) {
        return Type.getTypeDescriptor(this.getIndexParam(indexType, op, assignType, 'returnType'));
    },

    getIndexSetTemplate: function(op, indexType, assignType) {
        var t = this.getIndexParam(indexType, op, assignType, 'template');
    ...
    getIndexParam: function(indexType, operator, assignType, param) {

        var ops = this.operators.index;
        if (assignType && operator) {

            var t = indexType.toString() + operator + assignType.toString();
        ...
        if (assignType) {
            throw new TypeError('Unsupported index operation: ' + this + '[' + indexType + '] ' + operator + ' ' + assignType);
    ...
    getRangeGetReturn: function(indexType) {
        // will return a list
        return this.getIndexParam(indexType, null, 'returnType');
    },

    getRangeSetReturn: function() {
