...
TypeDescriptor.prototype = {
    ...
    defineAssign: function(type, template) {
        this.assign[type] = Type.template(template);
    },

    getAssignTemplate: function(type) {

        if (this.assign.hasOwnProperty(type)) {
            return this.assign[type];

        } else {
            throw err.Assign(this, type);
        }
