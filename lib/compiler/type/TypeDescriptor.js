...
TypeDescriptor.prototype = {
    ...
    defineEmptyType: function(emptyType) {
        this.emptyType = emptyType;
    },

    hasEmptyType: function(type) {
        if (!type) {
            console.log(type);
        }
        if (this.emptyType) {
            return this.emptyType.toString() === type.toString();
    ...
    defineInfixOp: function(op, rightType, returnType, template, defineAssign, custom) {
        ...
        if (defineAssign) {
            if (custom) {
                this.defineAssignment(op, rightType, '$1 = ' + template);
